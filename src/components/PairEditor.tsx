...
export function PairEditor({ entry, nodes, canDeleteApproved = false }: { entry: CatalogTransition; nodes: CatalogNode[]; canDeleteApproved?: boolean }) {
  ...
  const removePair = () => { if (!window.confirm(`Delete “${entry.title}” and all of its sound changes? An administrator can restore it from history.`)) return; startMove(() => { const data = new FormData(); data.set("transitionId", entry.id); return deletePairAction(data); }); };
  ...
  return <form action={editPairAction} className="pair-editor"><input type="hidden" name="transitionId" value={entry.id} /><input type="hidden" name="transitionRevision" value={entry.revision} />
    ...
    <div className="pair-editor__rows">{rows.map((row, index) => { const editable = canDeleteApproved || !row.id || row.approvalStatus === "pending"; return <div className={`rule-fields${editable ? "" : " rule-fields--locked"}`} key={`${row.id}-${index}`}>
      ...
      {(canDeleteApproved || !row.id || row.approvalStatus === "pending") && <button type="button" className="remove-rule" onClick={() => { if (window.confirm("Remove this sound change? An administrator can restore it from history after saving.")) setRows((current) => current.filter((_, rowIndex) => rowIndex !== index)); }} aria-label="Remove sound change">×</button>}
    </div>; })}</div>
